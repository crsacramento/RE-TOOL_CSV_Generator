...
public class BrowserHandler implements NativeKeyListener{
    ...
    public static void main(String[] args) {
        ...
        new File(System.getProperty("user.dir")+"\\HTMLfinal"+"\\").mkdirs();
        ...
        Filesystem.saveToFile("temp",Integer.toString(HTMLFileIndex),getPageSource(driver),false);
        HTMLFileIndex++;
        pageInfo.add(new PageInfo(driver.getCurrentUrl(), correlationBetweenSeleniumStepAndHtmlFiles));
    ...
    private void ProcessSeleniumActions() throws IOException {
        ...
        ArrayList<ExtendedSeleniumIDEElement> actions= new ArrayList<ExtendedSeleniumIDEElement>();
        ...
        try {
         ...
         //useful for, when a clickAndWait action occurs, to know if there was one or more types before it
         ...
         for (ExtendedSeleniumIDEElement element: actions)
         {
             if(element.getAction().equals("type"))
             {
                 ...
                 {
                     //if there is already an input in that textbox and it finds another one
                     //the Selenium step index is replaced by the most recent
                     //in this way, only the most recent string of each text box is analysed
             ...
             if(element.getAction().equals("clickAndWait"))
             {
                ...
                {
                    SeleniumHTMLInteraction.testForPatterns(actions,lastTypeIndexes, actions.indexOf(element)+1, pageInfo);
                }
             ...
             //at the end of Selenium steps, checks for types that were not analysed for input patterns
             if(actions.get(actions.size()-1).equals(element) && element.getAction().equals("clickAndWait")!=true)
             {
                 SeleniumHTMLInteraction.testForInputs(actions, lastTypeIndexes, actions.indexOf(element)+1, pageInfo);
             }
    ...
    private void ProcessList() {
        ...
        for(Patch patch: pageChanges)
        {
            ...
            Filesystem.saveToFile("final", Integer.toString(file), changes,false);
            file++;
        }

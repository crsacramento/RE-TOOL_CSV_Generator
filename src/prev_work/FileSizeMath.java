...
public class FileSizeMath {
    ...
    public static double calculateFileSizeByFileIndexAndCompareWithAverage(
            int i, double average) {
        File file = new File(System.getProperty("user.dir") + "\\HTMLfinal\\"
                + i + ".txt");
    ...
    public static double compareHTMLSizeWithPreviousFile(int index1, int index2) {
        File file = new File(System.getProperty("user.dir") + "\\HTMLtemp\\"
                + index1 + ".txt");
        double filesize = (double) file.length();
        // System.out.println("AAAAAAA: " + filesize);

        File fileRevised = new File(System.getProperty("user.dir")
                + "\\HTMLtemp\\" + index2 + ".txt");

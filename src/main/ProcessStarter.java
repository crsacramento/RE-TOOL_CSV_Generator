...
public class ProcessStarter {
    ...
    public static void startReverseEngineeringProcess(String[] args) {
        ...
        if (args.length != 1 && args.length != 2) {
            ...
            if(args.length == 2){
                ...
                try {
                    ...
                    System.err.println("ERROR: folder path passed as parameter is invalid");                    
                    e.printStackTrace();
                    return;
                }
                if(!f.exists())
                    f.mkdir();
                if(!f.isDirectory()){
                    System.err.println("ERROR: folder path must point to a directory");
                    return;
                }
        ...
        else WebsiteExplorer.initialize(baseURL);
